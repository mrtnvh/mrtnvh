			...
			install(vm) {
				vm.mixin({
					...
					computed: {
						$eq() {
							if (
								...
								typeof this.$data.$_elementQueryMixin_size
									.width === "number" &&
								typeof this.$data.$_elementQueryMixin_size
									.height === "number"
							) {
					...
					watch: {
						...
						"$data.$_elementQueryMixin_eq": function ({
							breakpoints,
						} = {}) {
							if (breakpoints) {
					...
					methods: {
						...
						$_elementQueryMixin_checkAllConditions(bp) {
							...
							return !Object.keys(bp).find(
								...
								(key) =>
									!this.$_elementQueryMixin_checkCondition(
										key,
										bp[key],
									),
							);
...
export default async (_context, inject) => {
	...
	if (process.server) {
		inject("eq", () => ({ small: false, desktop: false }));
	}
};
